 ...
 *
 * ⚠️ 2026-09 기준 **더 이상 동작하지 않는다.**
 *
 *   배포한 뒤 화면의 시세가 통째로 비어서 쫓아가 보니 Stooq 쪽이 막혀 있었다.
 *   브라우저에서 직접 확인한 결과다 — 우리 서버 IP 문제가 아니다.
 *
 *     stooq.com/q/l/    → 404  (시세 주소가 없어졌다)
 *     stooq.pl/q/l/     → 404  (본진 도메인에서도 없어졌다)
 *     stooq.com/q/d/l/  → Access denied  (주소는 있는데 접근을 막는다)
 *
 *   마지막 줄이 핵심이다. 404 가 아니라 *거부* 라는 건 주소는 살아 있는데
 *   자동 접근을 차단한다는 뜻이다. 되살릴 방법이 없다.
 *
 *   이 파일은 지우지 않고 남겨 둔다. 다른 제공사를 붙일 때 이 모양(카탈로그 id →
 *   심볼 표, 묶어 부르기, N/D 를 0 으로 바꾸지 않기)을 그대로 쓰면 되기 때문이다.
 *   되살아나면 다시 쓸 수도 있다.
 *
 * ── 아래는 막히기 전의 기록 ──────────────────────────────────────────
 *
 * 왜 이곳이었나
 *  - 가입도 키도 없이 S&P 500 · 나스닥 · 다우 · 러셀 · VIX · 환율을 한 형식으로 줬다.
 *  - CSV 한 줄이라 응답 모양이 잘 바뀌지 않았다. 그래서 무료 소스 중 가장 덜
 *    깨진다고 보고 골랐는데, **제공사가 통째로 문을 닫는 경우**는 그 판단 밖이었다.
 *    무료 소스 하나에 시세 전부를 걸면 이렇게 된다는 기록으로 남긴다.
 *
 * 지연 (중요)
